...
function countDataUrls(value, depth = 0) {
  if (depth > 12) return 0;
  if (typeof value === "string") return value.startsWith("data:") ? 1 : 0;
  if (Array.isArray(value)) return value.reduce((total, item) => total + countDataUrls(item, depth + 1), 0);
  if (!value || typeof value !== "object") return 0;
  return Object.values(value).reduce((total, item) => total + countDataUrls(item, depth + 1), 0);
}
...
class MediaStore {
  ...
  async saveDataUrl(value) {
    ...
    if (!content.length || content.length > 20 * 1024 * 1024) throw new Error("Media payload is too large");
...
async function migrateStoreMedia(store, mediaStore) {
  const current = store.read();
  const migratedItems = countDataUrls(current);
  if (!migratedItems) {
    return { changed: false, migratedItems: 0, revision: store.revision };
  }

  const externalized = await mediaStore.externalize(current);
  externalized.__revision = current.__revision;
  const result = await store.write(externalized);
  return {
    changed: true,
    migratedItems,
    revision: result?.revision || store.revision
  };
}
